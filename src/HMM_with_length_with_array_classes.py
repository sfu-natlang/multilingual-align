...
def initializeUniformly(N, num_classes): # K is the number of all possible values for Ys
    a = zeros((N+1,N+1,num_classes+2, N+1))
    #manager = Manager()
    #a = dict()
    pi = zeros(N+1)
    for i in range(1,N+1):
...
def smooth_transition_probabilities(a, I):
    for i in range(1, I+1):
        for j in range(1,I+1):
            a[i][j][I] = smooth_factor*(1.0/I) + (1 - smooth_factor) * a[i][j][I]
...
def readClasses(fileName, num_classes, e_count):
    ...
    print 'dict size' , len(word_classes_dict)
...
#                     else:
            ...
            for i in range(1,I+1):
                for j in range(1,I+1):
                    for class_of_e_i in range(2,num_classes+2):
                        a[(i,j,class_of_e_i,I)] = totalC_j_Minus_iOverAllObservations[(i,j,class_of_e_i,I)]/totalC_l_Minus_iOverAllObservations[i,class_of_e_i,I]
        for k in range(sd_size):
            (f,e) = biword[k]
        ...
        endTime = time.time()
        print "run time for one iteration of hmm_with_length_with_array model %.2gs" % (endTime - startTime)
        print iterations
...
def baumWelchP(bitext_sd, s_count,t_table,sd_count, d_count):#L is the number of observations
    ...
    a_array = Array(ct.c_double,((N+1)*(N+1)*(num_classes+2)*(N+1)))
    a_array2 = np.frombuffer(a_array.get_obj()) # mp_arr and arr share the same memory
    a = a_array2.reshape((N+1,N+1,num_classes+2,N+1)) # b and arr share the same memory
    ...
    for iterations in range(0,5):
        ...
        for i in range(N+1):
            for j in range(N+1):
                for l in range(num_classes+2):
                    for k in range(N+1):
                        totalC_j_Minus_iOverAllObservations[i,j,l,k] = 0.0
        '''
        manager = Manager()
        ...
        for i in range(N+1):
            for j in range(N+1):
                for l in range(num_classes+2):
                    totalC_l_Minus_iOverAllObservations[(i,l,j)] = 0.0
        '''
        manager2 = Manager()
	...
	#print 'a ', a
        ...
        for k in range(sd_size):
            ...
            t_table[(f,e)] = totalGammaDeltaOverAllObservations_t_overall_states[k]/totalGammaDeltaOverAllObservations_t_overall_states_over_dest[e]
        endTime = time.time()
        print "run time for one iteration of hmm_with_length_with_array parallel model %.2gs" % (endTime - startTime)
...
def log_viterbi(a,t_table,pi,N,o,d,word_classes_dict):
    V = zeros((N+1,len(o)))
    ptr = zeros((N+1,len(o)))
    ...
    for t in xrange(1,len(o)):
        for q in range(1,N+1):
            ...
            for q_prime in range(1,N+1):
                C_d_q_prime = word_classes_dict[d[q_prime-1]]
                if (o[t],d[q-1]) in t_table:
                    if a[(q_prime,q,C_d_q_prime,N)] != 0 and t_table[(o[t],d[q-1])] != 0 :
                        if V[(q_prime,t-1)]+log(a[(q_prime,q,C_d_q_prime,N)])+log(t_table[(o[t],d[q-1])]) > maximum :
                            maximum = V[(q_prime,t-1)]+log(a[(q_prime,q,C_d_q_prime,N)])+log(t_table[(o[t],d[q-1])])
                            max_q = q_prime
            V[q,t] = maximum
            ptr[q,t] = max_q
    max_of_V = -sys.maxint
...
def findBestAlignmentsForAll_AER(bitext,a,t_table,pi,num_lines,alignmentFile,word_classes_dict):
    alignment = open(alignmentFile,'w')
    for (n,(S,D)) in enumerate(bitext):
        N = len(D)
        bestAlignment = log_viterbi(a, t_table, pi, N, S, D,word_classes_dict)
        for (i,a_i) in enumerate(bestAlignment):
