...
def initializeUniformly(N): # K is the number of all possible values for Ys
    twoN = 2*N;
    a = zeros((twoN+1,twoN+1,N+1))      #This should be checked N+1 not 2N+1
    #manager = Manager()
    #a = dict()
    pi = zeros(twoN+1)
        ...
        for I in target_length_map:
            for i in range(1,I+1):
                for j in range(1,I+1):
                    a[(i,j,I)] = (1-p0H)*totalC_j_Minus_iOverAllObservations[(i,j,I)]/totalC_l_Minus_iOverAllObservations[i,I]
...
def baumWelchP(bitext_sd, s_count,t_table,sd_count):#L is the number of observations
    ...
    a_array = Array(ct.c_double,((twoN+1)*(twoN+1)*(N+1)))
    a_array2 = np.frombuffer(a_array.get_obj()) # mp_arr and arr share the same memory
    a = a_array2.reshape((twoN+1,twoN+1,N+1)) # b and arr share the same memory
    ...
    for iterations in range(0,5):
        ...
        for I in target_length_map:
            for i in range(1,I+1):
                for j in range(1,I+1):
                    if totalC_l_Minus_iOverAllObservations[i,I] == 0 :
                        print 'zero denom ', i, ' ', I
                    a[(i,j,I)] = (1-p0H)*totalC_j_Minus_iOverAllObservations[(i,j,I)]/totalC_l_Minus_iOverAllObservations[i,I]
        ...
        for q in range(1,2*N+1):
            ...
            for q_prime in range(1,2*N+1):
                if (o[t],newd[q-1]) in t_table:
                    if a[(q_prime,q,N)] != 0 and t_table[(o[t],newd[q-1])] != 0 :
                        if V[(q_prime,t-1)]+log(a[(q_prime,q,N)])+log(t_table[(o[t],newd[q-1])]) > maximum :
                            maximum = V[(q_prime,t-1)]+log(a[(q_prime,q,N)])+log(t_table[(o[t],newd[q-1])])
                            max_q = q_prime
                elif newd[q-1] == 'null':
                    if a[(q_prime,q,N)] != 0 :
                        if V[(q_prime,t-1)]+log(a[(q_prime,q,N)])+log(null_emission_prob) > maximum :
                            maximum = V[(q_prime,t-1)]+log(a[(q_prime,q,N)])+log(null_emission_prob)
                            max_q = q_prime
